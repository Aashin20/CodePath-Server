...
async def generate_code(request: GenerateCode):
    try:
        if not request.prompt or not request.language:
            raise HTTPException(status_code=400, detail="Invalid input")

        supported_languages = ["javascript", "cpp", "c", "java", "python"]
        if request.language.lower() not in supported_languages:
            raise HTTPException(status_code=400, detail="Unsupported language")

        prompt_template = prompts[request.language.lower()]
        final_prompt = prompt_template.replace("{{QUESTION}}", request.prompt)
